            ...
            <Box>
                <Stack direction={"column"} marginTop={"31px"} justifyContent="stretch" alignItems="stretch" sx={{ position: 'absolute', left: '53px', top: '180px', overflow:'auto' }}>
                    {(props.valueName || props.valueCompany) ?
                        ...
                        : persons.personsList.map((pers, index) =>
                            props.sort === pers.presence || props.sort === 'none' ?
                                <Button sx={{ marginBottom: '70px' }} key={index.toString()} onClick={() => { dispatch(getEditingPersonIndex(index)); props.setPopupWindow('Edit') }}>
                                    <Stack direction={'row'}>
                                        <Typography sx={{
                                            position: 'absolute', left: '33px', color: '#000000', fontSize: 30, fontFamily: "OpenSans, sans-serif",
                                            fontWeight: 400
